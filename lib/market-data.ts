...
export async function fetchCandles(symbol: string, timeframe: Timeframe): Promise<{ candles: Candle[]; currency: string }> {
  ...
  return { candles, currency: result?.meta?.currency ?? "IDR" };
}
